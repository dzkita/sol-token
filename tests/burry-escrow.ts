...
describe("burry-escrow", () => {
  ...
  it("Retiro de fondos cuando se rompe el feed y retiramos la cosa?", async () => {
    let didFail = false;
    ...
    // send tx
    try {
      const tx = await program.methods.withdrawClosedFeed()
      .accounts({
        user: payer.publicKey,
        escrowAccount: escrowState,
        closedFeedAccount: solUsedSwitchboardFeed,
        // systemProgram: anchor.web3.SystemProgram.programId
    }).signers([payer]).rpc()
      ...
      didFail = true;
      console.log(e.error.errorMessage)
      assert(e.error.errorMessage == 'Feed is not closed thus, should break at some point')
    }
